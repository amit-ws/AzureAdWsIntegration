...
public interface CustomRoleAssignmentRepository extends JpaRepository<CustomRoleAssignment, Integer> {
    @Query(value = "INSERT INTO custom_azure_role_assignment (assignee, scope, azure_role_definition_path_id, azure_id, principal_type, scope_type, description, status, created_at, expiry_time_amount, ws_tenant_name, ws_azure_subscription_id) " +
            ...
            "DO UPDATE SET azure_id = excluded.azure_id, principal_type = excluded.principal_type, scope_type = excluded.scope_type, description = excluded.description, status = excluded.status, created_at = excluded.created_at, expiry_time_amount = excluded.expiry_time_amount, ws_tenant_name = excluded.ws_tenant_name, ws_azure_subscription_id = excluded.ws_azure_subscription_id " +
            "RETURNING *",
            nativeQuery = true)
    CustomRoleAssignment saveOrUpdate(@Param("assignee") String assignee, @Param("scope") String scope, @Param("azureRoleDefinitionPathId") String azureRoleDefinitionPathId, @Param("azureId") String azureId,
                                      ...
                                      @Param("createdAt") Date createdAt, @Param("expiryTimeAmount") Long expiryTimeAmount, @Param("wsTenantName") String wsTenantName, @Param("subscriptionId") Integer subscriptionId);
