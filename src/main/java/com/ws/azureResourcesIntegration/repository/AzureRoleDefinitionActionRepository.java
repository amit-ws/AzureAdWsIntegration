...
public interface AzureRoleDefinitionActionRepository extends JpaRepository<AzureRoleDefinitionAction, Integer> {
    ...
    @Query(value = "SELECT arda.\"action\" as actionName, arda.\"type\" as actionType FROM azure_role_definition_action arda WHERE " +
            "arda.ws_azure_tenant_id  = :azureTenantId "
    , nativeQuery = true)
    List<AzureRoleDefinitionActionNameProjection> findAllAzureRoleDefinitionActionNamesByAzureTenantId(Integer azureTenantId);
}
