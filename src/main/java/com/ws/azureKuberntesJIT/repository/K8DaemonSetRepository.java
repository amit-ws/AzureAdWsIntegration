...
public interface K8DaemonSetRepository extends JpaRepository<K8DaemonSet, Long> {
    ...
    @Modifying
    @Query("DELETE FROM K8DaemonSet WHERE wsTenantName = :wsTenantName AND cloudProviderType = :cloudType AND (:cloudIds IS NULL OR cloudResourceAccountId IN :cloudIds)")
