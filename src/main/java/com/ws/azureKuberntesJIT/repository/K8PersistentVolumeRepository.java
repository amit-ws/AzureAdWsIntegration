...
public interface K8PersistentVolumeRepository extends JpaRepository<K8PersistentVolume, Long> {
    @Modifying
    @Query("DELETE FROM K8PersistentVolume WHERE wsTenantName = :wsTenantName AND cloudProviderType = :cloudType AND (:cloudIds IS NULL OR cloudResourceAccountId IN :cloudIds)")
